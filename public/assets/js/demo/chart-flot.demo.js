function showFlotTooltip(x, y, contents) {
  $('<div id="tooltip" class="flot-tooltip">' + contents + "</div>")
    .css({ top: y, left: x + 35, opacity: 0.8 })
    .appendTo("body")
    .fadeIn(200);
}
  ...
  if ($("#tracking-chart").length !== 0) {
    var plot = $.plot(
      ...
      ],
      {
        ...
        crosshair: { mode: "x", color: app.color.red },
        grid: {
          hoverable: true,
          autoHighlight: false,
          borderColor: "rgba(" + app.color.darkRgb + ", .15)",
        ...
        xaxis: { tickColor: "rgba(" + app.color.darkRgb + ", .15)" },
        legend: { show: true },
      }
    );
...
var handleBarChart = function () {
  ...
  if ($("#bar-chart").length !== 0) {
    ...
    $.plot(
      "#bar-chart",
      [{ label: "Bounce Rate", data: data, color: app.color.componentColor }],
      {
        series: {
          bars: {
            show: true,
...
var handleInteractivePieChart = function () {
  ...
  if ($("#interactive-pie-chart").length !== 0) {
    ...
    var colorArray = [
      app.color.orange,
      "rgba(" + app.color.componentColorRgb + ", .5)",
    ...
    $.plot($("#interactive-pie-chart"), data, {
      series: { pie: { show: true, stroke: { color: app.color.componentBg } } },
...
var handleLiveUpdatedChart = function () {
  ...
  if ($("#live-updated-chart").length !== 0) {
    ...
    var plot = $.plot(
      $("#live-updated-chart"),
      [{ label: "Server stats", data: getRandomData() }],
      {
        series: {
          shadowSize: 0,
          color: app.color.success,
